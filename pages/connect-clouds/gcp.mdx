...
description: Connect a GCP project to the AI Orchestrator using Workload Identity Federation — keyless, no service account JSON required.
diataxis: how-to
---
...
<Callout type="warning">the `--attribute-mapping` value must be one continuous line. Shell soft-wrap or pasting from a document that auto-wraps long lines introduces invisible newlines/spaces that gcloud rejects with a confusing parser error. If you hit that, assign to a variable first and `echo "$ATTR_MAPPING"` to verify no spaces or newlines before passing it with `--attribute-mapping="$ATTR_MAPPING"`.</Callout>
...
<Callout type="info">This binding is scoped to your specific org_id
...
claim, which doesn't match this binding, so GCP STS rejects them.</Callout>
