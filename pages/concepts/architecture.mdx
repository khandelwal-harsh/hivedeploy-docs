...
[ Backend API ]  ─── FastAPI at backend.hivedeploy.in
   ...
   │
   └──► Anthropic API  (the LLM doing the reasoning per agent turn)
                │
   ┌────────────┘
   ▼
...
4. Backend constructs the LLM prompt:
   ...
   - User's message
5. Backend calls Anthropic API (Claude)
...
Per-message latency is dominated by Anthropic API streaming
...
configurations evaluated by the same Anthropic API call. The
...
At chat time, the backend assembles {system_prompt} + {memory
relevant to current gate} + {session history} + {user message} →
